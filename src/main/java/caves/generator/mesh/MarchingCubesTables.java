 ...
 * <p>
 * The "hard" part about marching cubes is the number of possible edge combinations (256). The
 * purpose of this utility class is to solve that issue.
 * <p>
...
public final class MarchingCubesTables {
     ...
     * <p>
     * Value of <code>0x00</code> is used to indicate that the cube being sampled is completely
     * inside or outside the isosurface and thus does not have any visible vertices.
     */
    ...
    /**
     * Lookup table of series of three indices, indicating which vertices should be used at any
     * given cube index.
     */
    public static final int[][] TRIANGULATION_TABLE = {
            {},
            {0, 8, 3},
            {0, 1, 9},
            {1, 8, 3, 9, 8, 1},
    ...
    /**
     * Converts sample values from corners of the cubes into a cube index which can then be used to
     * index the {@link #EDGE_TABLE}.
     * <p>
     * Corner indices are clockwise, lower layer first. E.g. an edge between the indices 0 and 4
     * would be one vertical corner edge of the cube, 1-5 another, and so forth.
     *
     * @param surfaceLevel sample surface level threshold
     * @param samples      corner density samples
     *
     * @return index to the {@link #EDGE_TABLE}
     */
    public static int calculateCubeIndex(
        ...
        for (var i = 0; i < indexSizeInBits; ++i) {
            final var bitmask = 1 << i;
            if (samples[i] < surfaceLevel) {
                cubeIndex |= bitmask;
            }
        }
